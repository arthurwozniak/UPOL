...
(defmethod check-window (window)
  (error "Funkce knihovny micro-graphics volána s nesprávným odkazem na okno (~s)" window))

(defmethod check-window ((window mg-offscreen-window))
  t)

(defmethod check-window ((window mg-interface))
  t)

(defmethod apply-in-p-p :before (pane fun &rest args)
  (check-window pane))

(defmethod apply-in-p-p (pane fun &rest args)
  (check-window pane))
...
(defun draw-circle (window x y r)
  (apply-in-p-p 
   ...
   #'gp:draw-circle (pane window) (round x) (round y) (round r) :filled (filledp window))
  nil)
