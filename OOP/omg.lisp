...
(defmethod mouse-up ((shape shape) button position)
  (send-event shape 'ev-mouse-up shape button position))

(defmethod mouse-move ((shape shape) button position)
  (send-event shape 'ev-mouse-move shape button position))
...
(defmethod do-set-items ((shape compound-shape) value)
  ;; Zrušení okna a delegáta vypouštěným objektům:
  (send-to-items shape #'set-window nil)
  (send-to-items shape #'set-delegate nil)
  ;; Kopírování seznamu objeků, nastavení okna, delegáta a událostí:  
...
(defmethod do-set-items ((p picture) items)
  ...
  (send-to-items p #'add-event 'ev-mouse-up)
  (send-to-items p #'add-event 'ev-mouse-move)
  (send-to-items p #'add-event 'ev-dir-released)
  items)
...
(defmethod mouse-up ((p picture) button position)
  (let ((item (find-if (lambda (it) 
			 (contains-point-p it position)) 
		       (items p))))
    (when item
...
(defmethod ev-mouse-up 
    ((p picture) sender origin button position) 
  (send-event p 'ev-mouse-up origin button position))

(defmethod mouse-move ((p picture) button position)
  (let ((item (find-if (lambda (it) 
			 (contains-point-p it position)) 
		       (items p))))
    (when item
      (mouse-move item button position))))

(defmethod ev-mouse-move
    ((p picture) sender origin button position) 
  (send-event p 'ev-mouse-move origin button position))
...
(defun pt-in-seg-p (pt x1 x2 tolerance)
  "Zjistí, zda je bod pt na úsečce [x1 x2]."
  (let* ((u (vec-- x2 x1))
         (v (vec-- x1 pt))
...
(defun vert-between-p (pt pt1 pt2)
  (let ((pty (second pt))
        (pt1y (second pt1))
        (pt2y (second pt2)))
    ;; Nastavení optimalizace překladače (možná není nutné, lze bez následků vypustit)
...
(defmethod send-mouse-up ((w window) button x y)
  (window-mouse-up w
		     button 
		     (move (make-instance 'point) x y)))

(defmethod send-mouse-move ((w window) button x y)
  (window-mouse-move w
		     button 
		     (move (make-instance 'point) x y)))
...
; nainstalování callbacku pro mouse-up
  (mg:set-callback 
   (slot-value w 'mg-window) 
   :mouse-up (lambda (mgw button x y)
		 (declare (ignore mgw))
		 (send-mouse-up w button x y)))
; nainstalování callbacku pro mouse-move
  (mg:set-callback 
   (slot-value w 'mg-window) 
   :mouse-move (lambda (mgw button x y)
		 (declare (ignore mgw))
		 (send-mouse-move w button x y)))
  w)
...
(defmethod window-mouse-up ((w window) button position)
  (when (contains-point-p (shape w) position)
    (mouse-up (shape w) button position)))

(defmethod window-mouse-move ((w window) button position)
  (when (contains-point-p (shape w) position)
