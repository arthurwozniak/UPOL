...
class databaseOperations:
    ...
    def Ed(self, M):
        ...
        if M == []:
            result = set((i,j) for i in range(self.data_size) for j in range(i+1))
            self.eds[Mname] = result
            del(result)
            return self.eds[Mname]

        # najdu největší vypočítanou množinu S | S <= M
        ...
        for str_i in keys:
            ...
            if ((self.__subset_p(i, M)) and (self.eds[str_i] != None)):
                key = str_i
                break
