...
class AdaptiveCoder:
    ...
    def encode_integer(self, character):
        ...
        self.size += 1
        #print("encode: %s" % character)
        # print(self.size)
        rng = (self.up - self.lp + 1) // self.cumul_freq[self.symbols]
        # přičteme jedničku kvůli posunutí indexu
        self.up = self.lp + rng * self.cumul_freq[character + 1] - 1
        self.lp = self.lp + rng * self.cumul_freq[character]
        ...
        if self.up < self.lp:
            print(self.size)
            raise Exception("Lower bound is bigger than higher.")
        # škálování
    ...
    def decode_stream(self, size, encoded, output_file):
        ...
        while True:
            ...
            for i in range(0, self.symbols):
                if self.cumul_freq[i] <= c < self.cumul_freq[i + 1]:
                    sym = i
                    break
            ...
            if sym is None:
                ...
                print("x value: %s" % x)
                print("X cumulative value: %s" % ((x - self.lp) // rng))
                tmp = ((x - self.lp) / ((self.up - self.lp + 1) /
                                        self.cumul_freq[self.symbols]))
