...
unsigned int pocet_dnu(unsigned int mesic)
{
    static const int pocet[] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
    if (mesic <= 12)
...
int vyhledej(char *soubor, char *kriteria, ...)
{
    ...
    kriteria_pocet = strlen(kriteria);

    if(kriteria_pocet==0)return -1;

    va_start(param, kriteria);
    while(kriteria_pocet>0){
        switch(kriteria[i]){
            case 'j': jmeno = va_arg(param, char*);break;
            case 'p': prijmeni = va_arg(param, char*);break;
            case 'r': rok = va_arg(param, int);break;
            case 'm': mesic = va_arg(param, int);break;
            case 'd': den = va_arg(param, int);break;
            case 's': stav = va_arg(param, int);break;
            case 'P': pohlavi = va_arg(param, int);break;
            default: break;
        }
        i++;
        kriteria_pocet--;
    ...
    va_end(param);
    file = fopen(soubor, "rb");
    while(fread(&tmp_osoba, sizeof(osoba), 1, file) == 1){

        if((jmeno != NULL) && (strcmp(jmeno, tmp_osoba.jmeno) != 0))continue;
        if((prijmeni != NULL) && (strcmp(prijmeni, tmp_osoba.prijmeni) != 0))continue;
        if((stav != 0) && (stav != tmp_osoba.stav))continue;
        if((pohlavi != 0) && (pohlavi != tmp_osoba.pohlavi))continue;
        if((den != 0) && (den != tmp_osoba.narozen.den))continue;
        if((mesic != 0) && (mesic != tmp_osoba.narozen.mesic))continue;
        if((rok != 0) && (rok != tmp_osoba.narozen.rok))continue;
        vypis(tmp_osoba);
    }
